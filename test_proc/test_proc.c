...
static ssize_t procf_read(char *buffer, char **buffer_location, off_t offset, int buffer_length, int *eof, void *data)
...
static const struct file_operations mf_proc_file_fops = {
 .owner = THIS_MODULE,
 .open  = open_callback,
 .read  = read_callback,
};
...
int init_module() {
    ...
    mf_proc_file = proc_create(PROCF_NAME, 0644, NULL,mf_proc_file_fops);
